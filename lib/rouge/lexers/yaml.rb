module Rouge
  module Lexers
    class YAML < RegexLexer
      ...
      def reset_indent
        debug { "    yaml: reset_indent" } if $ROUGE_DEBUG
        @indent_stack = [0]
        @next_indent = 0
      ...
      def save_indent(match)
        ...
        debug { "    yaml: indent: #{self.indent}/#@next_indent" } if $ROUGE_DEBUG
        debug { "    yaml: popping indent stack - before: #@indent_stack" } if $ROUGE_DEBUG
        if dedent?(@next_indent)
          @indent_stack.pop while dedent?(@next_indent)
          debug { "    yaml: popping indent stack - after: #@indent_stack" } if $ROUGE_DEBUG
          debug { "    yaml: indent: #{self.indent}/#@next_indent" } if $ROUGE_DEBUG
      ...
      def continue_indent(match)
        debug { "    yaml: continue_indent" } if $ROUGE_DEBUG
        @next_indent += match.size
      end
